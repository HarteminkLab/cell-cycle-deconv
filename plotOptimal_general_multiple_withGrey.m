function [model] = plotOptimal_general_multiple_withGrey(shortlist, f, drawLine)
...
elseif nargin == 1
	drawLine = 1;
	f = 'allgenes/allgenes.f';
elseif nargin ~= 3
	error('Expected 3 arguments: list of genes, deconvolved profile file (optional), draw centroid line (optional)');
end
...
orig_genes = textread(shortlist, '%s');
...
overallF = zeros(1, size(F,2));
for i=1:n_genes
	cid = orfids{i};
	curf = F(cid, :);
%	maxf = max(curf([t_y_idx b_y_idx]));
%	curf = curf./maxf;
	allF = [allF; curf];
	overallF = overallF + curf;
end

overallF = overallF./n_genes;
...
overall_t_y = overallF(t_y_idx);
overall_t_y = overall_t_y./max(overall_t_y);
plot(t_x, overall_t_y, 'color', 'r', 'linewidth', 0.5);
...
overall_b_y = overallF(b_y_idx);
overall_b_y = overall_b_y./max(overall_b_y);
plot(b_x, overall_b_y, 'color', 'r', 'linewidth', 0.5);
