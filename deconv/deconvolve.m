function [model] = deconvolve(model)
	...
	cvx_begin
        ...
        % TODO: Here is the code to debug the size of the f and H matrices
        % we can use this if the dimensions don't appear as they should
        % 
        % disp(['Size of f: ', num2str(size(f(1:Hsize)))]);
        % disp(['Size of H: ', num2str(size(H))]);
        % disp(['Size of g: ', num2str(size(g))]);
        % H*f(1:Hsize);
        ...
        % The minimization that can have errors with dimensions is
        % typically the first statement H*f ./ g'
        % Note f is padded, so we subset
        %
        % An example of the expected dimension of the norm's first
        % argument:
        %
        %   (30 x 256) (256 x 1)     ./    (30 x 1)  => 30 x 1
        %
		minimize(...
