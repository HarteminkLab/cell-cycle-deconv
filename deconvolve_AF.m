function [model] = deconvolve_AF(orfname, modeltype, datatype, alpha, gamma, learn_flag, fig_flag)
...
if nargin < 6
	error('... need 6 inputs: orfname, datatype, alpha, gamma, learn_flag, fig_flag');
end
...
elseif strcmp(datatype, DECONV_JOINT)
	...
	error('Wrong datatype (expect AF1/AF2/JOINT)');
end
...
if learn_flag == 1
	SLIENCE = 0;
	[model] = findOptimal4AF(model, fig_flag);
else
	SLIENCE = 0;
	model.gm = gamma;
