...
REPLICATE=1
for ITR in {1..4}; do
    OUTDIR="output/combined_deconvolve_replication_rep${REPLICATE}_${ITR}"
	ARGS="${OUTDIR} ${REPLICATE} ${CHROM} ${NUM_EPOCHS}"
    sbatch -D ./slurm-logs/ --job-name="rep${REPLICATE}_${ITR}" -p compsci --export="PYFILE=src/SingleReplicateDeconvolutionRunner.py,ARGS=$ARGS" scripts/cpu_job.sh
done
