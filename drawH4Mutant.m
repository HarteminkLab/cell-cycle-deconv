function drawH4Mutant(modeltype, datatype, alpha)
...
DATA_MT1 = strcat(DECONV_DATASET, 'mt1.txt');
DATA_MT2 = strcat(DECONV_DATASET, 'mt2.txt');

SMALL = 1e-8;

if nargin < 1
	error('... Expect one inputs: datatype(MT1/MT2/JOINT)');
end
...
% MT1
if strcmp(datatype, DECONV_MT1)
	modelfile = strcat(MODEL_DIR, MT1_DIR, modelprefix, '.', int2str(alpha(1)), '.label'); 
	if exist(modelfile, 'file') ~= 2
		error(sprintf('%s does not exist', modelfile));
	end

	dataset = load(DATA_MT1, 'ascii');
	g = dataset(orfid,:)';
	model.g{1} = g;
	model.timepoints{1} = MT1_TP;
...
%%% datatype = WT2
elseif strcmp(datatype, DECONV_MT2)
	modelfile = strcat(MODEL_DIR, MT2_DIR, modelprefix, '.', int2str(alpha(1)), '.label');
	if exist(modelfile, 'file') ~= 2
		error(sprintf('%s does not exist', modelfile));
	end

	dataset = load(DATA_MT2, 'ascii');
	g = dataset(orfid,:)';
	model.g{1} = g;
	model.timepoints{1} = MT2_TP;
...
elseif strcmp(datatype, DECONV_JOINT)
	% deal with MT1
	modelfile1 = strcat(MODEL_DIR, MT1_DIR, modelprefix, '.', int2str(alpha(1)), '.label');
	if exist(modelfile1, 'file') ~= 2
		error(sprintf('%s does not exist', modelfile1));
	end

	dataset1 = load(DATA_MT1, 'ascii');
	g1 = dataset1(orfid,:)';
	model.timepoints{1} = MT1_TP;
	model.g{1} = g1;
	...
	% deal with MT2 %
	% ============= %
	modelfile2 = strcat(MODEL_DIR, MT2_DIR, modelprefix, '.', int2str(alpha(2)), '.label');
	if exist(modelfile2, 'file') ~= 2
		error(sprintf('%s does not exist', modelfile2));
	end

	dataset2 = load(DATA_MT2, 'ascii');
	g2 = dataset2(orfid,:)';
	model.timepoints{2} = MT2_TP;
	model.g{2} = g2;
	...
	error('wrong data type (expect MT1/MT2/JOINT)');
end
...
for i=1:n
	...
	if strcmp(datatype, DECONV_MT1)
		fprintf('Time = %d: ', MT1_TP(i));
	elseif strcmp(datatype, DECONV_MT2)
		fprintf('Time = %d: ', MT2_TP(i));
	else
		if i<=n/2
			fprintf('Rep1 Time = %d: ', MT1_TP(i));
		else
			fprintf('Rep2 Time = %d: ', MT2_TP(i-n/2));
		end
	end
...
%	set(gca, 'ytick', ymax/2);
	...
	if strcmp(datatype, DECONV_MT1)
		str = sprintf('%d min', MT1_TP(i));
	elseif strcmp(datatype, DECONV_MT2)
		str = sprintf('%d min', MT2_TP(i));
	else
		if i<=n/2
			str = sprintf('Rep1: %d min', MT1_TP(i));
		else
			str = sprintf('Rep2: %d min', MT2_TP(i-n/2));
		end
	end
