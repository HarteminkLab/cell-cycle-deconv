function [model] = plotOptimal_general_branch_graph(model, fig_flag, imagedir)
...
%		end
	...
	for i = 1:length(model.iList)
		...
		len = length(list)-1;
		plot(i_x(idx:min(idx+len, end))'+halfbin, i_y(idx:min(idx+len,end)), 'LineWidth', 3, 'color', areacolor);
		idx = idx+len;
		hold on;
	end
...
%		titlename = sprintf('%s (%s; %s; %d; %0.3g)', orfname, model.datatype, mt, alpha(1), model.gm);
	...
	for i = 1:length(model.tList)
		...
		len = length(list)-1;
		halfbin = (list(2)-list(1))/2;
		plot(t_x(idx:min(idx+len,end))'+halfbin, t_y(idx:min(idx+len,end)), 'LineWidth', 3, 'color', areacolor);
		idx = idx+len;
		hold on;
	end
  ...
  ylim([0 y_lim]);
	...
	for i = 1:length(model.bList)
		...
		len = length(list)-1;
		halfbin = (list(2)-list(1))/2;
		plot(b_x(idx:min(idx+len,end))'+halfbin, b_y(idx:min(idx+len,end)), 'LineWidth', 3, 'color', areacolor);
		idx = idx+len;
		hold on;
	end
