function deconvList(genefile, modeltype, datatype, alpha, imagedir, gamma)

% ---------- %
NeedCheck = strcat(genefile, '.needcheck.genes');
NeedRedo = strcat(genefile, '.needredo.genes');
...
if nargin < 5
	error('... Expect at least inputs: genefile, modeltype, datatype, alpha, imagedir, gamma(optional)');
end
...
needcheck_fid = fopen(NeedCheck, 'w');
needredo_fid = fopen(NeedRedo, 'w');
...
for idx = 1:1:len_orfs
	...
	if exist('gamma') == 1 % fix gamma
		model.gm = gamma;
		[model] = deconvModel(model);
	else
		opt_flag = 1;
		[model, opt_flag] = findOptimal(model, 0);

		if opt_flag == 0
			fprintf(needcheck_fid, '%s\n', model.orig_orfname);
		end

		if model.rn > DEFAULT_RN_CUTOFF || model.fb_corr < FB_CORR_CUTOFF
			fprintf(needredo_fid, '%s\n', model.orig_orfname);
		end
