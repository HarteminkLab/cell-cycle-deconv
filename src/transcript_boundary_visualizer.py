...
# -------------------------------------------------------------
# Needs to be refactored. Uses outdated transcript calling code.
# Some of this can be salvaged.




# class TranscriptCallerVisualizer:
# 	"""
...
# 		if self.caller.pileup_data is None:
...
# 		if self.caller.data is None:
# 			raise ValueError("No pileup data available. Load data first.")
		
# 		# Compute average pileups and log transform
# 		watson_avg = self.caller.data.mean(axis=0).values
# 		crick_avg = self.caller.data.mean(axis=0).values
...
# 		end_idx = min(len(self.caller.data.columns), 
...
# 		watson_avg = self.caller.data.iloc[:, start_idx:end_idx].mean(axis=0).values
# 		crick_avg = self.caller.data.iloc[:, start_idx:end_idx].mean(axis=0).values
