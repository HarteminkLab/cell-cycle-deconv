...
sbatch -a 0-999%16 -D ./slurm-logs/ --job-name="genm_1" -p compsci --export="PYFILE=src/deconvolve_genome.py,ARGS=$ARGS,BATCH=0" scripts/cpu_genomicarray_job.sh
sbatch -a 0-216%16 -D ./slurm-logs/ --job-name="genm_2" -p compsci --export="PYFILE=src/deconvolve_genome.py,ARGS=$ARGS,BATCH=1" scripts/cpu_genomicarray_job.sh

# If each job takes 1 hour, and there are 1,216 jobs. If our throughput is 32 jobs at a time.
# How many hours will it take to finish? (1,216 jobs)  / (32 simultaneous jobs)  * (1 hour per job) 
# = 38 hours = 1.58 days
