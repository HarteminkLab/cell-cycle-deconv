function deconvList4AF(genefile, imagedir, gamma)
...
if nargin < 3
	error('... Expect 4 inputs: genefile, imagedir, gamma(optional, a numerical value; if 0, learn gamma; if -1, gnenfile specify gamma for each gene)');
end
...
prefix = strcat(genefile, '.AF.', datatype);
...
if gamma ~= -1
	...
	[orig_orfnames, names, gammas] = textread(genefile, '%s\t%s\t%f');
end
