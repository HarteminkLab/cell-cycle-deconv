...
class Config:
	...
	def get_H_dataframe_mapping(self):
		...
		dg1_df = df_for_phase_set(cg1_indices, dg1_tps, 'DG1')
	...
	def get_timepoints_phases_Hpositions_for_branch(self, branch):
		...
		for phase in phases_for_branch:
			...
			if phase == 'DG1':
				phase_columns = self.phase_columns['CG1']
			else:
				phase_columns = self.phase_columns[phase]

			timepoints = search_df[(search_df.phase == phase) & 
								   (search_df.branch == branch)].timepoint
			branch_indices.append((phase, timepoints[:-1], phase_columns))
