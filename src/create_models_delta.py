...
class ModelCreation:
	...
	def __init__(self, posteriors_filepath, output_model_path):
		...
		self.Rname = "RG1"
		self.CG1_intervals = "t 0 b 1"
		self.PG1_intervals = "i 1 t 1 b 2"
	...
	def get_sub_interval_str(self):
		...
		subnames = ['RG1', 'CG1', 'Delta', 'postG1']
		branches = ['i', 't', 'b']

		intervals_str = ""

		# Keep track of the interval string for each branch
		# keep in an array so it can be sorted before converting into a string
		branch_intervals = {'i': [], 't': [], 'b': []}
			...
			for subname in subnames:
				...
				for subintervals_dict in subintervals:
					...
					if k == br:
						...
						branch_intervals[br].append(interval)

		# Convert the branch mapping into an intervals string
		ret_branch_string = ""
		for branch in branches:
			current_branch_intervals = branch_intervals[branch]

			# Sort the intervals in each branch
			current_branch_intervals = sorted(current_branch_intervals, key=lambda arr: arr[0])

			# Create the intervals string for the branch
			ret_branch_string += f"# {branch}\n"
			intervals_str = ""
			for interval in current_branch_intervals:
				intervals_str += ' '.join([f"{x:.4f}" for x in interval]) + '\n'

			ret_branch_string += intervals_str

		return ret_branch_string
