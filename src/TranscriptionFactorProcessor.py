...
class TranscriptionFactorProcessor:
    ...
    def _load_tf_datasets(self):
        ...
        cc_rossi_tfs = rossi_sites[
            (rossi_sites.tf.str.upper().isin(self.binding_sites.kelliher_rossi_intersection)) &
            (rossi_sites.value == 1000)
        ].set_index('identifier')
        
        # Sort by chromosome and start position for efficient loading
        self.tf_sites = cc_rossi_tfs.sort_values(['chr', 'start'])
    ...
    def _prepare_binding_sites(self):
        ...
        self._validate_coordinates(self.tf_sites)
        
        # Add window coordinates for analysis
        self.tf_sites['window_start'] = self.tf_sites['start'] - self.window_size
        self.tf_sites['window_end'] = self.tf_sites['stop'] + self.window_size
        
        print(f"Prepared {len(self.tf_sites)} binding sites with ±{self.window_size}bp windows")
    
    def _validate_coordinates(self, sites_df):
        """
        Ensure binding site coordinates are valid.
        
        Parameters:
        -----------
        sites_df : pd.DataFrame
            DataFrame containing binding site coordinates
        """
        # Basic coordinate validation
        invalid_sites = sites_df[
            (sites_df['start'] < 0) | 
            (sites_df['stop'] <= sites_df['start'])
        ]
        
        if len(invalid_sites) > 0:
            print(f"Warning: Found {len(invalid_sites)} sites with invalid coordinates")
