...
class ViolinPlotPlotter():
	...
	def plot_box(self, box_plot_data, violin_data, x_location, group_index, ax):

		q1, q3, median, mean, lower_whisker, upper_whisker, outliers = box_plot_data
		...
		x_center = x_location # Center of the group of violins
		# Total width
		...
		if self.plot_whiskers:
			plt.plot([x_location, x_location], [lower_whisker, q1], color='black', zorder=2, lw=0.75)
			plt.plot([x_location, x_location], [q3, upper_whisker], color='black', zorder=2, lw=0.75)
		...
		def compute_density_curve(data, bw=0.001, mult=1, linspace=None):
			...
			def _kde_sklearn(x, x_grid, bandwidth):
				...
				pdf = np.exp(log_pdf)
				return pdf
			if linspace is None: linspace = min(data), max(data), 500
	...
	def plot_box_plot(self, ax, title):
		"""Plot a violin of a dataframe with a quantile column to separate
...
def get_violin_data_for_metric_df(dat, subselect_index, data_key, binstep=1, index_range_key='H_range'):
	...
	# Narrow format for the violinter
