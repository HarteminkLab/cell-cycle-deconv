...
% ============ %
if learn_flag == 1
	[model] = findOptimal_small(model, fig_flag);
else
	model.gm = gamma;
	if ~SLIENCE
...
%	[model] = boundarySmooth(model);
	[model] = plotOptimal_general_branch(model, fig_flag);
	[model] = plotOptimal_general_branch_graph(model, fig_flag);
%	[model] = plotOptimal_general_branch_area(model, fig_flag);
%	[model] = plotOptimal_general_parts(model, fig_flag);
end
