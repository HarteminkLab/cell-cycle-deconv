function [gammas, rn, sn, final_gm] = findOptimal_small(model, fig_flag)
...
if flag
	...
	if (abs(gm_right-gm_left) < SMALL) % gm_right == gm_left
		...
		if log_space
			r_gm = log2(gm_right);
			l_gm = log2(gm_left);
			step = (r_gm-l_gm)/ELBOW_BINS;
			gamma_array = l_gm:step:r_gm;
			gamma_array = 2.^gamma_array;
		else
			step = (gm_right-gm_left)/ELBOW_BINS;
			gamma_array = gm_left:step:gm_right;
		end
		[model.gm, bs_flag, gammas, rn, sn] = findElbow(model, gamma_array, fig_flag);
	end
...
while right-left > LR_SMALL
	...
	if ~SLIENCE
		disp(sprintf('  ...   gm = %0.4g, rn = %0.4f, rate = %0.1f', model.gm, model.rn, rn_rate));
	end
