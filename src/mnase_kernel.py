...
class MNaseSeqDensityKernel:
	...
	def __init__(self, extent=[-55, 55, 0, 260], 
		mean_length=163, var_length=200,
	...
	def compute_kernel(self):
		...
		kernel_mat_down_sampled = downsample_bins(self.kernel_mat.reshape((1, 
		    *self.kernel_mat.shape)), 
		    (self.extent[0], self.extent[1]), (self.extent[0], self.extent[1]), 10, 10, self.extent[3])

		self.kernel_mat_down_sampled = kernel_mat_down_sampled[0]
	...
	def plot(self):
		...
		plt.subplot(1, 2, 1)
		plt.imshow(self.kernel_mat, extent=self.extent, 
				origin='lower', cmap='RdYlBu_r', aspect='auto')
		plt.colorbar()

		plt.subplot(1, 2, 2)
		plt.imshow(self.kernel_mat_down_sampled, cmap='RdYlBu_r', extent=self.extent, 
